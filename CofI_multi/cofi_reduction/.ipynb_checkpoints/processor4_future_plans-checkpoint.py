...
class CofiProcessor1:
    ...
    def multi_extract2d(self, red, trace, targets, imcr, clobber=False,
                    # --- Main control parameters ---
                    param_area=None, output=None,output_2=None, update_callback=None,
        ...
        def process_slitlet(o, targ, adjust_wavelength, display_obj
                           ):
            wav = spectra.WaveCal(f'./CofIwav_{o.header['OBJNAME']}_{targ["ID"]}.fits') #'CofIwav_{:s}_{:s}.fits'.format(arc.header['OBJNAME'],targ['ID'])
            orig = wav.model.c0_0
            wav.add_wave(o)
            if adjust_wavelength:
                trace1 = spectra.Trace(file=trace_file, transpose=trace_transpose, lags=trace_lags, # lags seems hardcoded here
                                      ...
                                      ) # Use configured degree/sigdegree
                ...
                peak, ind = trace1.findpeak(o, thresh=findpeak_thresh, width=center, sc0= findpeak_sc0, plot= findpeak_plot, # Use configured thresh
                ...
                if display_obj:
                    display_obj.tv(o)
                o = wav.correct(o, o.wave[nrows // correcting_value])
                if display_obj:
                    display_obj.tv(o)
                name = o.header["FILE"].split(".")[0]
                o.write(f'{name}_{targ["ID"]}_2d.fits', overwrite=True)
                return wav.model.c0_0 - orig
            else:
                nrows = o.shape[0]
                o = wav.correct(o, o.wave[nrows // correcting_value])
                name = o.header["FILE"].split(".")[0]
                o.write(f'{name}_{targ["ID"]}_not_adjusted_2d.fits', overwrite=True)
                plt.figure()
        ...
        def on_click_run(b):
            with output_2:
                ...
                for i, (o, targ) in enumerate(zip(out, targets)):
                    name_prefix = o.header["FILE"].split(".")[0]
                    if adjust_wavelength:
                        outname = f'{name_prefix}_{targ["ID"]}_2d.fits'
                    else:
                        outname = f'{name_prefix}_{targ["ID"]}_not_adjusted_2d.fits'

                    if not clobber and os.path.exists(outname):
                        print(f"Skipping extraction for {targ['ID']}, file '{outname}' already exists.")
                        # Optionally load the existing data if needed downstream
                        out[i] = image.rd(outname,ext=1)
                        continue
