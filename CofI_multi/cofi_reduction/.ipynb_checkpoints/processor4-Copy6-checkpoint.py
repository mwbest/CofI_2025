...
class CofiProcessor1:
    ...
    def multi_extract2d(self, red, trace, targets, imcr,
        ...
        def _draw_selection_lines(display_obj, peak_row, extraction_rad, sky_rows, ncols):
            ...
            display_obj.ax.plot([0, ncols], [upper_science, upper_science], color='cyan', linestyle='--', linewidth=1, label='Science Aperture')
            display_obj.ax.plot([0, ncols], [lower_science, lower_science], color='cyan', linestyle='--')
            ...
            if sky_rows:
                ...
                display_obj.ax.plot([0, ncols], [first_row, first_row], color='red', linestyle=':', linewidth=0.5, alpha=0.7, label='Sky Rows')
                # Draw the rest of the sky rows without a label
                for row_index in range(1, len(sky_rows)):
                    row = sky_rows[row_index]
                    display_obj.ax.plot([0, ncols], [row, row], color='red', linestyle=':', linewidth=0.5, alpha=0.7)
            ...
            if by_label:
                display_obj.ax.legend(by_label.values(), by_label.keys(), loc='upper right', fontsize='small', framealpha=0.7)
                ...
                def process_slitlet(o, targ):
                    ...
                    if adjust_wavelength:
                        ...
                        def run_skyline_and_prompt(current_rad):
                            ...
                            # 2. Run skyline without its own plotting to avoid clearing the display.
